...
info = """
[\033[91mSIN\033[00m] \033[91mCortex, Made By Envy, Re Coded from PEIXOTO,
i liked this scripts, so i simply re coded it,
i will be adding new methods all the time,
stay tuned. 
PEIXOTOs Biggest Attack
31.9 gbps
Cortexs, Biggest Attack,
Not Recorded, (Most Likely The Same As PEIXOTO.)
"""
...
║ \033[00mExit                          \033[91m|\033[00m EXIT PEIXOTO\033[91m         ║
...
║ \033[00mInfo                          \033[91m|\033[00m DISPLAY PEIXOTOS INFO\033[91m║
╚══════════════════════════════════════════════════════╝\033[00m
"""
...
║ \033[00mStats                         \033[91m|\033[00m DISPLAY PEIXOTO STATS\033[91m║
╚══════════════════════════════════════════════════════╝\033[00m
"""
...
╚══════════════════════════════════════════════════════╝\033[00m"""
banner = """\033[1;00m
 ꧁ঔৣ☬✞ P E I X O T O--M D Z Z ✞☬ঔৣ꧂
 ꧁ঔৣ☬✞ P E I X O T O--M D Z Z ✞☬ঔৣ꧂
 ꧁ঔৣ☬✞ P E I X O T O--M D Z Z ✞☬ঔৣ꧂         \033[1;91m: い\033[00m
"""

altbanner = """
			     Angels go to heaven
			   Demons meet the gates of hell
		      PEIXOTO people are punished put in hell
		     Sinners Meet The Cortex And Fall Into The Vortex
		      		       	C O R T E X  
"""

cookie = open(".PEIXOTO_cookie","w+")
...
def synsender(host, port, timer, punch):
	...
	aid += 1
	tattacks += 1
	while time.time() < timeout and telenet and attack:
...
def main():
	...
	while True:
		sys.stdout.write("\x1b]2;P E I X O T O\x07")
		sin = input("\033[1;00m[\033[91mPEIXOTO\033[1;00m]-\033[91m家\033[00m ").lower()
		sinput = sin.split(" ")[0]
		if sinput == "clear":
			os.system ("clear")
			print (altbanner)
			...
			sfound = 0
			sys.stdout.write("\x1b]2;P E I X O T O |{}| F O U N D\x07".format (sfound))
			try:
				host = sin.split(" ")[1]
				with open(r"/usr/share/PEIXOTO/subnames.txt", "r") as sub:
			...
			try:
				...
				while True:
					try:
						...
						sys.stdout.write("\x1b]2;P E I X O T O |{}ms| D E M O N S\x07".format (stop))
						print ("PEIXOTO: {}:{} | Time: {}ms [\033[91mUP\033[00m]".format(ip, port, stop))
						sock.close()
						time.sleep(1)
					except socket.error:
						sys.stdout.write("\x1b]2;P E I X O T O |TIME OUT| D E M O N S\x07")
						print ("PEIXOTO: {}:{} [\033[91mDOWN\033[00m]".format(ip, port))
						time.sleep(1)
					except KeyboardInterrupt:
						print("")
						main()
